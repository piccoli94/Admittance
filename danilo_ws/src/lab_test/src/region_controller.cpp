...
void    torque_control(                         
 ...
 /*       Delta_x=x_e-xdes;
	...
	for(int i=0; i<7; i++){
        ...
        s=dq-q_r_dot;
        if (!first_iter)	q_r_dot_dot << 0,0,0,0,0,0,0;            
	else	derivate_dqr();
	...
	std::cout << "theta_cap: " << theta_cap.maxCoeff() << std::endl;
        ...
        std::cout << "tau_c : "<< tau_m_c.transpose() << std::endl;
...
*/
        ...
        jacobiano();
        j_pseudoinverse();
        //std::cout << "pinv_j : "<< pinv_J << std::endl;
        q_r_dot = ( pinv_J * x_f_dot - alpha * pinv_J * k_p * delta_x );
	...
	derivate_dqr();
        ...
        regressore_Yd();
        //std::cout << "max Y_d: " << Y_d << std::endl;
        theta_estimate();


        tau_m_a= -J.transpose()*k_p*delta_x;
        //std::cout << "tau_a: " << tau_m_a.transpose() << std::endl;

        tau_m_c= Y_d * theta_cap;
        //std::cout << "tau_c: " << tau_m_c.transpose() << std::endl;

        tau_m_b = - k_s * s;
        //std::cout << "tau_b: " << tau_m_b.transpose() << std::endl;
...
Matrix<double, 7, 1>    get_tau_a(){ return  tau_m_a; }
Matrix<double, 7, 1>    get_tau_b(){ return tau_m_b;  }
Matrix<double, 7, 1>    get_tau_c(){ return tau_m_c;  }
Matrix<double, 7, 1>    get_q(){ return  q; }
Matrix<double, 7, 1>    get_dq(){ return dq;  }
Matrix<double, 7, 1>    get_ddqr(){ return q_r_dot_dot;  }
Matrix<double, 7, 1>    get_dqr(){ return q_r_dot;  }
