...
void    pos_stiffness_tank_tiny()
{
    ...
    else if (region.compare("R-DR") == 0 && norm_Delta_x <= (a_r+a_h)/2)
    {
	k_p= k_max * 0.5 * ( 1 - cos ( M_PI * ( delta_x.norm() - a_h ) / ( 0.5*a_r + 0.5*a_h  - a_h ) ) );
    }
    else if (region.compare("R-DR") == 0 && norm_Delta_x >  (a_r+a_h)/2)
    {
	k_p= k_max * 0.5 * ( 1 + cos ( M_PI * ( delta_x.norm() - ( 0.5*a_r + 0.5*a_h ) ) / ( a_r - (0.5*a_r + 0.5*a_h )) ) );
    }
    else if (region.compare("TS-SR") == 0)
    {
        k_p= 0;
...
REGION_CONTROLLER( Matrix<double, 3, 1> x_i,
                        ...
                        REGION_CONTROLLER::_k_chain=_k_chain;

    	_fksolver = new KDL::ChainFkSolverPos_recursive( REGION_CONTROLLER::_k_chain ); // crea nuovo oggetto cinematica inversa relativo alla catena
	_q_in = new KDL::JntArray( REGION_CONTROLLER::_k_chain.getNrOfJoints() ); //crea l'array relativo ai giunti
	_dyn_param = new KDL::ChainDynParam(REGION_CONTROLLER::_k_chain,KDL::Vector(0,0,-9.81));
	_jsolver = new KDL::ChainJntToJacSolver(REGION_CONTROLLER::_k_chain);
	J0.resize(REGION_CONTROLLER::_k_chain.getNrOfJoints());
	G=new KDL::JntArray(REGION_CONTROLLER::_k_chain.getNrOfJoints());
